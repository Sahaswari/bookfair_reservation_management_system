...
type ApiFetchOptions = RequestInit & { skipAuth?: boolean; retry?: boolean; rawResponse?: boolean };

export function apiFetch<T>(path: string, options?: ApiFetchOptions & { rawResponse?: false }): Promise<ApiResponse<T>>;
export function apiFetch<T>(path: string, options: ApiFetchOptions & { rawResponse: true }): Promise<T>;
export async function apiFetch<T>(
  path: string,
  options: ApiFetchOptions = {},
): Promise<ApiResponse<T> | T> {
  const { skipAuth, retry, headers, rawResponse, ...rest } = options;
  ...
  if (rawResponse) {
    if (!response.ok) {
      const message = (json as ApiResponse<T>).message || "Request failed";
  ...
  if (!response.ok || typed.success === false) {
